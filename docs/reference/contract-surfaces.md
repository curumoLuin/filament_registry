...
- **Checklista breaking change**: Klucze obiektów w `p_lines` (`filament_id`, `filament_name_snapshot`, `estimated_usage_g`) muszą zgadzać się co do znaku z listą kolumn w `jsonb_to_recordset` — `jsonb_to_recordset` daje dla nieznanego klucza `NULL`, a nie błąd. Dla `filament_name_snapshot` i `estimated_usage_g` łapie to NOT NULL, dla `filament_id` łapie to dopiero porównanie `row_count` z `jsonb_array_length` (to samo, które odrzuca szpulę należącą do innego konta). Nigdy nie wracaj do dwóch osobnych insertów z kompensacją w kodzie: niepodzielność ma wynikać z transakcji, a nie z tego, czy kod sprzątający zdążył się wykonać (R-10). Funkcja zgłasza `NO_LINES` — ten sam prefiks co `set_project_status` — oraz `NOT_AUTHENTICATED` i `LINES_NOT_IN_INVENTORY`. Insert idzie przez `join public.filaments`, bo klucz obcy jest egzekwowany przez systemowy trigger zwolniony z RLS: bez joina cudza szpula przeszłaby. Nie usuwaj też obu `revoke execute …` — `create function` nadaje EXECUTE roli PUBLIC automatycznie, a Supabase dokłada domyślne uprawnienie dla `anon`; po migracji `proacl` ma być `{postgres=X, authenticated=X, service_role=X}`.

## INSUFFICIENT_QUANTITY / FILAMENT_MISSING / NO_LINES / LINES_NOT_IN_INVENTORY (prefiksy błędów RPC)

- **Kanoniczna definicja**: `supabase/migrations/0001_init.sql` — `INSUFFICIENT_QUANTITY: …`, `FILAMENT_MISSING: …`, `NO_LINES`, `INVALID_STATUS: …`, `PROJECT_NOT_FOUND`; `supabase/migrations/0002_atomic_project_creation.sql` — `NO_LINES`, `NOT_AUTHENTICATED`, `LINES_NOT_IN_INVENTORY: <ile> of <ze ilu>`.
...
- **Checklista breaking change**: Przeredagowanie komunikatu w Postgresie sprawia, że użytkownik widzi surowy błąd bazy. Zmieniaj oba miejsca razem albo przejdź na dopasowanie po `errcode` zamiast po treści. `LINES_NOT_IN_INVENTORY` niesie liczbę, a nie nazwę szpuli — dlatego ma własną gałąź w `humanise()` i własny kształt ładunku; nie sklejaj go z `FILAMENT_MISSING`. Pokryte przez `tests/unit/db-errors.test.ts`.
