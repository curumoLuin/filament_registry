...
- **Checklista breaking change**: Klucze obiektów w `p_lines` (`filament_id`, `filament_name_snapshot`, `estimated_usage_g`) muszą zgadzać się co do znaku z listą kolumn w `jsonb_to_recordset` — `jsonb_to_recordset` daje dla nieznanego klucza `NULL`, a nie błąd. Dla `filament_name_snapshot` i `estimated_usage_g` łapie to NOT NULL, dla `filament_id` łapie to dopiero porównanie `row_count` z `jsonb_array_length` (to samo, które odrzuca szpulę należącą do innego konta). Nigdy nie wracaj do dwóch osobnych insertów z kompensacją w kodzie: niepodzielność ma wynikać z transakcji, a nie z tego, czy kod sprzątający zdążył się wykonać (R-10). Funkcja zgłasza `NO_LINES` — ten sam prefiks co `set_project_status` — oraz `NOT_AUTHENTICATED` i `LINES_NOT_IN_INVENTORY`. Insert idzie przez `join public.filaments`, bo klucz obcy jest egzekwowany przez systemowy trigger zwolniony z RLS: bez joina cudza szpula przeszłaby. Nie usuwaj też `revoke execute … from public` — `create function` nadaje EXECUTE roli PUBLIC automatycznie.
